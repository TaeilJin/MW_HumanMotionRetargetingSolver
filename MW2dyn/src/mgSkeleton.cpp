...
int mgSkeleton::getWMatrixAt(int boneId, CoordinateType* data, gXMat& wMat, double scale)
{
...
int mgSkeleton::getLMatrixAt(int boneId, CoordinateType* data, gXMat& lMat, double scale)
{
	gRotMat tRot;
	gVec3 transVec;
	mgBone *bone = bones[boneId];
	...
	lMat.setRot( bone->H.rot() * tRot );
	lMat.setTrn( bone->H.rot() * transVec +  bone->H.trn() );

	return 0;
}

int mgSkeleton::getLMatrixAt(gXMat& lMat, int boneId, const CoordinateType* data, double scale) const
{
	gRotMat tRot;
	gVec3 transVec;
	mgBone *bone = bones[boneId];
