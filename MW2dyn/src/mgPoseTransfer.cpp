...
int		mgPoseTransfer::transferPoseLevMar(gXMat& offset)
{
	...
	//update tarPoints
	for(int i=0;i<tarPoints.size();++i) tarPoints[i].updateKinematicsUptoPos();
	
	//desired direction vectors from srcCharacter
	int sz = directions.size()*3 + positions.size()*3 + tar->dof() - 6;	
	...
	dlevmar_dif( objFuncLevMar, p, NULL, m, sz, 50000, LevMarOpts, LevMarInfo, NULL, NULL, this);
